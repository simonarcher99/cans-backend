...
class CanItemViews(APIView):
    ...
    def post(self, request):
        request.data['item'] = request.data['item'].strip().title()
    ...
    def patch(self, request, pk):
        ...
        if int(quantity) < 0:
            return Response({'status': 'error', 'message': 'Can not have a negative quantity'}, status=status.HTTP_502_BAD_GATEWAY)
        print(quantity)
